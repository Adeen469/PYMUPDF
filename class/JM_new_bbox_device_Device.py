"""
Class: fitz.JM_new_bbox_device_Device

Description:
Wrapper class for struct fz_device with virtual fns for each fnptr; this is for use as a SWIG Director class.

Inheritance (MRO): JM_new_bbox_device_Device -> FzDevice2 -> FzDevice -> object

"""

import fitz

# ===== Methods and Attributes of fitz.JM_new_bbox_device_Device =====
...
# __init__(self, result, layers)  [function]
#     -> == Constructor.
...
# fill_image(dev, ctx, image, ctm, alpha, color_params)  [function]

# fill_image_mask(dev, ctx, image, ctm, colorspace, color, alpha, color_params)  [function]

# fill_path(dev, ctx, path, even_odd, ctm, colorspace, color, alpha, color_params)  [function]

# fill_shade(dev, ctx, shade, ctm, alpha, color_params)  [function]

# fill_text(dev, ctx, text, ctm, *args)  [function]
...
# ignore_text(dev, ctx, text, ctm)  [function]
...
# stroke_text(dev, ctx, text, stroke, ctm, *args)  [function]
